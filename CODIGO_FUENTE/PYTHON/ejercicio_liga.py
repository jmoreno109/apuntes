...
import csv
def LeerPartidos():
	"""FunciÃ³n que lee el fichero CSV y devuelve los datos del mismo en una 
...
def Equipos(liga):
	"""FunciÃ³n que recibe la lista de diccionarios con los datos de la liga y devuelve un
	 conjunto con los equipos de la liga."""
	return(tuple(set([partido["equipo1"] for partido in liga])))


def QuienGana(resultado):
	"""FunciÃ³n que recibe un resultado y devuelve un 0 si es un empate, un 1 si
...
def Puntos(info):
	"""* `Puntos(info)`: FunciÃ³n que recibe una lista con los partidos ganados,
	 empatados y perdidos y devuelve los puntos obtenidos."""
	return 3*info[0]+info[2]

def InfoEquipos(liga,*equipos):
	"""FunciÃ³n que recibe la lista de diccionarios con los datos de la liga y el conjunto de
